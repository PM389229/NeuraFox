...
# Supprimer le warning de PyTorch
warnings.filterwarnings("ignore", category=UserWarning)

def generate_tts(text_to_speak, output_file_path):
    """
    Génère un fichier audio MP3 à partir du texte français fourni.
    """
    try:
        # Utilise le GPU si disponible, sinon le CPU
        device = "cuda" if torch.cuda.is_available() else "cpu"
        model = MeloTTS(language='FR', device=device)
        model.speaker_id = model.hps.data.spk2id['FR']
